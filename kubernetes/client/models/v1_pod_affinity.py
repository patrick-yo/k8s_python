...
class V1PodAffinity(object):
    ...
    def required_during_scheduling_ignored_during_execution(self):
        ...
        If the affinity requirements specified by this field are not met at scheduling time, the list will not be scheduled onto the node. If the affinity requirements specified by this field cease to be met at some point during list execution (e.g. due to a list label update), the system may or may not try to eventually evict the list from its node. When there are multiple elements, the lists of nodes corresponding to each podAffinityTerm are intersected, i.e. all terms must be satisfied.  # noqa: E501
