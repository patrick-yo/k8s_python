...
class V1beta1NetworkPolicySpec(object):
    ...
    def ingress(self):
        ...
        List of ingress rules to be applied to the selected pods. Traffic is allowed to a list if there are no NetworkPolicies selecting the list OR if the traffic source is the list's local node, OR if the traffic matches at least one ingress rule across all of the NetworkPolicy objects whose podSelector matches the list. If this field is empty then this NetworkPolicy does not allow any traffic (and serves solely to ensure that the pods it selects are isolated by default).  # noqa: E501
